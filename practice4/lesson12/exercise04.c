...
void insert_before_even(element_type **pp, int val);
...
int main(void) {
  ...
  printf("これから整数を入力し、その整数をリスト中の最初の偶数の前に挿入します。\n");
  printf("操作は繰り返されます｡\n");
...
/*   ここに関数 insert_before_even を記述します。   */
void insert_before_even(element_type **pp, int val){
	element_type *p = *pp, *new_even, *new, *next;
	while(p->next->value % 2 == 1){
		p = p->next;
		if(p->next == NULL){
			new_even = (element_type *)malloc(sizeof(element_type));
			new_even->value = 2;
			new_even->next = NULL;
			p->next = new_even;
		}

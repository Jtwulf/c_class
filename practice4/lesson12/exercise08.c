...
void insert_before_all_even(element_type **pp, int val);
...
int main(void) {
  ...
  printf("整数を入力し、その値をリスト中の全ての偶数の前に挿入します。\n");
  printf("要素値を入力＞ ");
  getword(word, BUFSIZE);
  num = atoi(word);
  insert_before_all_even(&root, num);
  putlist(root);
  putchar('\n');
...
/*   ここに関数 insert_before_all_even を記述します。   */

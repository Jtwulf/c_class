...
element_type *insert_pos(element_type *top, int val, int pos);
...
int main(void) {
  ...
  int pos;
  scanf("%d",&pos);
  printf("リストに挿入する要素を入力 -> ");
  int val;
  scanf("%d",&val);
  insert_pos(root, val, pos);

  printf("挿入後のリストを出力します。");
  putlist(root);
...
element_type *insert_pos(element_type *p, int val, int pos){
	if(pos == 0){
		element_type *new;

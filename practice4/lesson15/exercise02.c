...
int count_single_child_node(node_type *r);
...
int main(void) {
  ...
  show_tree(root);
  k = count_single_child_node(root);
  printf("この木における子供を1つだけ持つノードの個数は %d です。\n", k);
  free_node(&root);

  return 0;
}

/*  ここに関数 count_single_child_node  を記述します  */
int count_single_child_node(node_type *r){
	if(r->left != NULL && r->right != NULL){
		return count_single_child_node(r->right) + count_single_child_node(r->left);
	} else if(r->left != NULL)
		return 1 + count_single_child_node(r->left);
	else if(r->right != NULL)
		return 1 + count_single_child_node(r->right);
}

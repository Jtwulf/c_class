...
int count_even_node(node_type *r);
...
int main(void) {
  ...
  show_tree(root);
  k = count_even_node(root);
  printf("この木の中の偶数のキーを持つ node の数は %d 個です。\n", k);
  free_node(&root);

  return 0;
}

/*   ここに関数 count_even_node を記述します。  */
int count_even_node(node_type *r){
	int right = 0, left = 0;
	if(r->right != NULL){
		right = count_even_node(r->right);
		if(r->right->key % 2 == 0)
			right ++;
		right --;
	}
	if(r->left != NULL){
		left = count_even_node(r->left);
		if(r->left->key % 2 == 0)
			left ++;
		left --;
	}

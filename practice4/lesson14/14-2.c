...
void search_max_min(node_type *root, node_type **max_pp, node_type **min_pp);
...
int main(){
	...
	node_type *max_node, *min_node;
	search_max_min(root, &max_node, &min_node);
	printf("最大のキー値を持つノードは key: %d, data: %s\n",max_node->key, max_node->data);
	printf("最小のキー値を持つノードは key: %d, data: %s\n",min_node->key, min_node->data);
...
void search_max_min(node_type *root, node_type **max_pp, node_type **min_pp){
	...
	if(r != NULL){
		...
		if(r->left != NULL)
			search_max_min(r->left, max_pp, min_pp);
		if(r->right != NULL)
			search_max_min(r->right, max_pp, min_pp);
	}
}
